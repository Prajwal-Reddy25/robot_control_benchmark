...
1. Use one committed YAML file; copy it into the evidence directory and record its SHA-256 digest
   in `provenance.json`.
...
| compute throughput | reciprocal of mean compute time, not the ROS loop frequency |
...
Use a fresh output directory. The runner overwrites matching artifacts but does not delete unrelated
files from a prior matrix; `summary.csv` and `REPORT.md` define the matrix generated by that run.
